...
class ChapterButton(QPushButton):
    ...
    def set_chapter(self, chapter: AbstractChapter) -> ChapterButton:
        ...
        if self.eye_icon:
            ...
            self.chapter._signals.is_read_changed.connect(lambda is_read: self.eye_icon.change_icon(is_read))
            self.eye_icon.pressed.connect(lambda: chapter.set_is_read(self.eye_icon._is_second_icon))
        return self