...
class ChapterImageLoader(QObject):
    placeholder_ready = Signal(str, int, int, QPixmap)  # manga id, chapter num, placement index, placeholder
    image_ready = Signal(str, int, int, str, bytes)     # manga id, chapter num, placement index, image filename, image
    ...
    def load_chapter(self, manga_id: str, chapter: MangaChapter):
        ...
        self.downloader.download_images(self.urls, convert=AppConfig.ImageDownloading.convert_image())
        
    def _metadata_downloaded(self, url: str, metadata: ImageMetadata):
        if url not in self._emited_metadata:
            placeholder = PlaceholderGenerator.static(metadata.width, metadata.height, f'{self.urls[url]} (#{self.urls_num[url]})')
    ...
    def _image_downloaded(self, url: str, name: str, metadata: ImageMetadata):
        self.image_ready.emit(self.manga_id, self.chapter.number, self.urls_num.pop(url), name, self.cache.get_image(name))
