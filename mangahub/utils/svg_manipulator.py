...
class SVGManipulator:
    ...
    def __init__(
        self, svg_file_path: Path | None = None, svg_content: str | None = None
    ):
        """
        ...
        for elem in elements:
            ...
            if fill or fill_if_none:
                if "fill" in elem.attrib and (
                    elem.get("fill").lower() != "none" or fill_if_none
                ):
            ...
            if stroke or stroke_if_none:
                if "stroke" in elem.attrib and (
                    elem.get("stroke").lower() != "none" or stroke_if_none
                ):
    ...
    def add_background(self, bg_color: str) -> "SVGManipulator":
        ...
        for rect in self.root.findall(".//svg:rect", self.namespaces):
            if (
                ...
                and rect.get("y", "0") == "0"
                and (
                    rect.get("width") == self.root.get("width")
                    or rect.get("width") == self.root.get("viewBox").split()[2]
                )
            ):  # type: ignore
    ...
    def resize(
        self,
        width: str | int | float = "SVGManipulator",
        ...
        if width is not None:
            self.root.set(
                "width", str(width) if isinstance(width, str) else f"{width}px"
            )
        if height is not None:
            self.root.set(
                "height", str(height) if isinstance(height, str) else f"{height}px"
            )
    ...
    def simplify(self) -> "SVGManipulator":
        ...
        try:
            ...
            # Remove metadata
            for metadata in lxml_tree.xpath(
                "//svg:metadata", namespaces=self.namespaces
            ):
