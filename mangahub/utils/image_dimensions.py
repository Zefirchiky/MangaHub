...
async def get_image_dimensions_from_header(session, url, max_bytes=1024):
    ...
    """
    try:
        async with session.get(url, headers={'Range': f'bytes=0-{max_bytes-1}'}) as response:
...
def _get_jpeg_dimensions(data):
    ...
    while True:
        ...
        if marker[0] == 0xFF and marker[1] in (0xC0, 0xC1, 0xC2, 0xC3, 0xC5, 0xC6, 0xC7, 0xC9, 0xCA, 0xCB, 0xCD, 0xCE, 0xCF):
            # Skip segment length
            stream.read(2)
            # Skip precision byte
            stream.read(1)
