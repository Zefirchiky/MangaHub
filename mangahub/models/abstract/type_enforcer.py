...
from typing import Any, Type, Sequence, Mapping, get_type_hints, get_origin, get_args
from types import UnionType
                    

class TypeEnforcer[T]:
    """Class will enforce `T` type on `value`"""
    def __init__(self, value: T, strongly_typed: bool=True, try_parse_after_failure=False):
        """
        Args:
    ...
    def __set_name__(self, owner: Type, name: str):
        ...
        if self.strongly_typed:
            ...
            parsed, val = self._parse_check_type(self._value, self._type, self.try_parse_after_error)
        ...
        if parsed:
            ...
            if self.strongly_typed:
                raise TypeError(f"Value of class {owner}.{name} must be of type {self._type}")
            else:
                raise TypeError(f"Value of class {owner}.{name} must be of type {self._type} or corresponding values must be able to convert into corresponding types.\n"
    ...
    def _check_type(cls, value, type_) -> bool:
        ...
        # Is complex type (aka dict[str, int], list[dict[str, str | int]])
            ...
            if isinstance(value, orig):
                if isinstance(value, list) or issubclass(orig, Sequence):
                    return all(map(lambda x: cls._check_type(x, args[0]), value))
                
                if isinstance(value, dict) or issubclass(orig, Mapping):
                    return all(map(lambda x: cls._check_type(x, args[0]), value.keys())) and \
                           all(map(lambda x: cls._check_type(x, args[1]), value.values()))
    ...
    @classmethod
    def _parse_check_type(cls, value, type_, ignore_error=False) -> tuple[bool, T | Any]:
        """
        ...
        supports simple types, union types, sequences, and mappings. If the conversion
        is successful, it returns the parsed value; otherwise, it returns the original value.
        ...
        # Is union (aka int | str)
            ...
            if isinstance(value, orig):
                if isinstance(value, list) or issubclass(orig, Sequence):
                    ...
                    parsed = True
                    for v in value:
                        values_parsed, val = cls._parse_check_type(v, args[0], ignore_error)
                        l.append(val)
                ...
                if isinstance(value, dict) or issubclass(orig, Mapping):
                    ...
                    for k, v in value.items():
                        keys_parsed, key = cls._parse_check_type(k, args[0], ignore_error)
                        values_parsed, val = cls._parse_check_type(v, args[1], ignore_error)
                        d[key] = val
