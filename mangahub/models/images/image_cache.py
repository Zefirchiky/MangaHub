...
class ImageCache:
    ...
    def add_image(self, name: str, image: bytes, size: int):
        if self.max_ram < size:             # If image larger that maximum ram available
            ...
            if self.cur_ram + size >= self.max_ram:     # If image takes more that ram available
                self._free_ram(size)
        ...
        if self._freed_from_ram:           # If files where moved from ram
            if self.cur_disc + self._freed_ram_data_bytes >= self.max_disc and self.max_disc >= self._freed_ram_data_bytes:   # If file takes more that disc space available
