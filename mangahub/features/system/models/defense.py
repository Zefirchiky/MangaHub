...
class Defense:
    ...
    def get_damage(self, damage: int, damage_type: DamageTypes=DamageTypes.NORMAL) -> int:
        damage = damage
        if DamageTypes.NORMAL in damage_type:
            damage -= self.defense if not self.type == DefenseTypes.PERCENTAGE else damage / 100 * self.defense
            
        if DamageTypes.TRUE in damage_type:
            if self.type == DefenseTypes.TRUE and Config.true_defense_over_true_damage:
                damage -= self.defense
            if self.type == DefenseTypes.PERCENTAGE and Config.percentage_defense_over_true_damage:
                damage -= damage / 100 * self.defense
                
        if DamageTypes.PERCENTAGE in damage_type:
            if self.type == DefenseTypes.TRUE and Config.true_defense_over_percentage_damage:
                damage -= self.defense
            if self.type == DefenseTypes.PERCENTAGE and Config.percentage_defense_over_percentage_damage:
                damage -= damage / 100 * self.defense
