...
class MangaViewerScene(QGraphicsScene):
    ...
    def __init__(self, parent: MangaViewer):
        ...
        self._placeholder_queue: list[tuple[int, int, int]] = []    # ! heapq is used for fast smallest element
    ...
    def add_placeholder(self, index: int, pixmap: QPixmap) -> None:
        heapq.heappush(self._placeholder_queue, (index, pixmap.width(), pixmap.height()))
            
        if not self._placeholder_timer.isActive():
            self._placeholder_timer.start(AppConfig.UI.MangaViewer.placeholder_loading_intervals())
    ...
    def _placeholders_load_finished(self):
        ...
        if self._placeholder_queue:
            ...
            return [i for i in range(self._cur_index + 1, indexes[-1]) if i not in indexes]
    ...
    def _cull_images(self, y: int | float, height: int | float, multiplier: float):
        ...
        for i in self._visible_image_indexes - indexes: # {1, 2, 3, 4} - {3, 4, 5, 6} = {1, 2}  Indexes that were visible, but not anymore
            item, width, height = self._image_items[i]
            item.setPixmap(QPixmap(width, height))
            item.setVisible(False)
            
        self.images_request.emit(indexes - self._visible_image_indexes) # {3, 4, 5, 6} - {1, 2, 3, 4} = {5, 6}  Indexes that are visible, but lack images
        self._visible_image_indexes = indexes
        
    def _get_image_items_in_range(self, y1, y2) -> set[int]:
        indexes = set()
        cur_y = -1
        for i, (image, height, _) in self._image_items.items(): # TODO: Binary tree will do better for this
            if cur_y != -1:
...
class MangaViewer(SmoothGraphicsView):
    ...
    def _on_images_request(self, indexes: set[int]):
        for i in indexes:
            try:
                image_bytes = self.parent_.app_controller.manga_manager.get_image(self._image_indexes_names.get(i, ''))
                image = QPixmap()
                image.loadFromData(image_bytes)
            except Exception:
                image = self.parent_.app_controller.manga_manager.get_placeholder(self.manga, self.chapter, i)  # type: ignore
    ...
    def _cull_scene(self):
        ...
        self._scene._cull_images(current, viewport_height, AppConfig.UI.MangaViewer.cull_height_multiplier())
        QTimer.singleShot(AppConfig.UI.MangaViewer.cull_scene_cooldown(), self._cull_cooldown_timer_out)
