...
class MangaSiteScraper:
    ...
    def get_last_chapter_num(self, site: Site, manga: Manga) -> int:
        if site.last_chapter_parsing.on_title_page:
            ...
            html = self.get_chapter_page(
                site, manga, site.last_chapter_parsing.on_chapter_page
            ).text

        regex_url = (
            site.last_chapter_parsing.string_format.replace(
                "$chapter_num$", r"(?P<chapter_num>[0-9]+)"
            )
            .replace("$manga_id$", manga.id_)
            .replace("$num_identifier$", r"\w+")
        )  # TODO
        ...
        if not chapter_nums:
            logger.warning(
                f"No match for {regex_url} ({site.last_chapter_parsing.string_format}) was found"
            )
            return 0
    ...
    def get_chapter_name(self, site: Site, manga: Manga, num) -> str:
        chapter_page = BeautifulSoup(
            self.get_chapter_page(site, manga, num), "html.parser"
        )
    ...
    def get_chapter_image_urls(self, site: Site, manga: Manga, num) -> list[str]:
        ...
        if not chapter_page:
            logger.warning(
                f"No chapter page for '{manga.name}' chapter {num} with site '{site.name}'"
            )
            return None
        ...
        if site.images_parsing.regex_from_html:
            ...
            for script in scripts:
                if script.string:
                    urls = re.findall(
                        re.compile(site.images_parsing.url_regex), script.string
                    )
                    if urls:
                        return urls
            ...
            urls = [
                img.get("src")
                for img in chapter_page.find_all(
                    "img",
        ...
        logger.warning(
            f"No image urls for '{manga.name}' chapter {num} with site '{site.name}'"
        )
        return None
    ...
    def get_chapter_placeholders(self, site: Site, manga: Manga, num: float) -> bytes:
        ...
        placeholders_worker.signals.all_completed.connect(
            lambda _: MM.show_success("Image sizes downloaded")
        )
        placeholders_worker.signals.error.connect(
            lambda error: MM.show_error(str(error), 5000)
        )
        placeholders = list(
    ...
    def get_chapter_images(self, site: Site, manga: Manga, num):
        ...
        images_worker.signals.all_completed.connect(
            lambda _: MM.show_success("Images downloaded")
        )
