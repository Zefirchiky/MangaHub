...
class MangaDexScraper:
    ...
    def get_manga_data(self, name):
        ...
        # MM.show_message(MM.MessageType.WARNING, f"Manga {name} was not found on MangaDex")
        return None
    ...
    def get_manga_cover_filename(self, manga_id):
        ...
        data = response.json()["data"]
        if not data:
            # MM.show_message(MM.MessageType.ERROR, f"Manga {manga_id} not found")
            return None
    ...
    def get_last_chapter_num(self, manga_id, language="en"):
        ...
        data = data.get("data")
        if not data:
            # MM.show_message(MM.MessageType.WARNING, f"Empty site response (data: {data}) for manga {manga_id}, when getting last chapter: {response.status_code}")
            return 0
    ...
    def get_chapter_data(self, manga_id, num, limit=1, language="en"):
        ...
        # MM.show_message(MM.MessageType.ERROR, f"Chapter {manga_id} {num} not found on MangaDex")
        return None
    ...
    def get_chapter_image_urls(self, chapter_id, data_saver=1) -> list[str]:
        try:
            ...
            # MM.show_message(MM.MessageType.ERROR, f"Chapter {chapter_id} not found: {response.status_code}")
            return []
        
        data = response.json()
        if not data:
            # MM.show_message(MM.MessageType.ERROR, f"Chapter {chapter_id} not found")
            return []
